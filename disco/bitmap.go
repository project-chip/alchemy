...
func (b *Baller) organizeBitmapSection(cxt *discoContext, bms *subSection) (err error) {
	...
	if err != nil {
		return fmt.Errorf("error renaming table header cells in section %s in %s: %w", bms.section.Name, cxt.doc.Path, err)
	}

	err = b.addMissingColumns(cxt, bms.section, bitsTable, matter.Tables[matter.TableTypeBitmap], types.EntityTypeBitmapValue)
	if err != nil {
		return fmt.Errorf("error adding missing table columns in bitmap section %s in %s: %w", bms.section.Name, cxt.doc.Path, err)
	}
