...
func (b *Baller) parseDoc(doc *spec.Doc, docType matter.DocType, topLevelSection *asciidoc.Section) (dp *docParse, err error) {
	...
	for section := range parse.FindAll[*asciidoc.Section](doc.Reader(), topLevelSection) {
...
func findSubsections(dp *docParse, parent *subSection, childPatterns ...subSectionChildPattern) (subSections []*subSection, err error) {
	...
	var i int
	for ss := range parse.Skim[*asciidoc.Section](dp.doc.Reader(), parent.section, parent.section.Children()) {
