...
func (r PreParser) Process(cxt context.Context, input *pipeline.Data[*DocGroup], index int32, total int32) (outputs []*pipeline.Data[asciidoc.Iterator], extras []*pipeline.Data[*DocGroup], err error) {
	var iterator asciidoc.Iterator
	iterator, err = preparse(input.Content, input.Content.Root, r.specRoot, r.attributes)
	if err != nil {
		return
	}
	input.Content.Iterator = iterator
	outputs = append(outputs, &pipeline.Data[asciidoc.Iterator]{Path: input.Content.Root.Path.Relative, Content: iterator})
	return
}
...
func preparse(docGroup *DocGroup, doc *Doc, specRoot string, attributes []asciidoc.AttributeName) (iterator asciidoc.Iterator, err error) {
	pps := &preparseState{
	...
	iterator = &PreParseIterator{elements: pps.actions}
	return
}
...
func preparseFile(pps *preparseFileState, docGroup *DocGroup, doc *Doc, parent asciidoc.Parent, els asciidoc.Elements) (err error) {
	...
	var addToCell bool
	parse.Traverse(asciidoc.NewRawIterator(), parent, els, func(el asciidoc.Element, parent asciidoc.Parent, index int) (should parse.SearchShould) {
		...
		case *asciidoc.UserAttributeReference:
			if el.Name() == "cluster" {
				slog.Info("replacing cluster attribute", log.Type("parentType", parent), "address", fmt.Sprintf("%p", el), "parentAddress", fmt.Sprintf("%p", parent), log.Path("source", el))
			}
		...
		case *asciidoc.Section:
			if suppress {
				...
				iterator := &PreParseIterator{elements: pps.state.actions}
...
type PreParseIterator struct {
	elements map[asciidoc.Element]*preparseAction
}

func (ppi *PreParseIterator) Iterate(parent asciidoc.Parent, elements asciidoc.Elements) asciidoc.ElementIterator {
...
func (ppi *PreParseIterator) Count(elements asciidoc.Elements) int {
	count := 0
	for _, el := range elements {
		action, ok := ppi.elements[el]
		if ok {
