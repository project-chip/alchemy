...
func (ri *referenceIndex) anchorId(iterator asciidoc.Iterator, parent asciidoc.Parent, element asciidoc.Element, id asciidoc.Elements) string {
	if existing, ok := ri.anchorIds[element]; ok {
		return existing
	}

	var s strings.Builder
	for el := range iterator.Iterate(parent, id) {
		switch el := el.(type) {
...
func (ri *referenceIndex) changeAnchor(anchor *Anchor, parent asciidoc.Parent, id asciidoc.Elements) {
	anchorID := ri.anchorId(anchor.Document.Iterator(), parent, anchor.Element, anchor.ID)
	...
	anchorID = ri.anchorId(anchor.Document.Iterator(), anchor.Parent, anchor.Element, id)
...
func (ri *referenceIndex) changeCrossReference(reference *CrossReference, id asciidoc.Elements) {
	referenceID := ri.anchorId(reference.Document.Iterator(), reference.Reference, reference.Reference, reference.Reference.ID)
	...
	referenceID = ri.anchorId(reference.Document.Iterator(), reference.Reference, reference.Reference, id)
