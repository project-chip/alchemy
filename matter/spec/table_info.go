...
func crossReferenceToDataType(doc *Doc, cr *asciidoc.CrossReference, isArray bool) *types.DataType {
	...
	id := doc.anchorId(doc.Iterator(), cr, cr, cr.ID)
	if len(cr.Elements) > 0 {
		var label strings.Builder
		for el := range doc.Iterator().Iterate(cr, cr.Children()) {
			switch el := el.(type) {
			case *asciidoc.String:
...
func (ti *TableInfo) ReadDataType(row *asciidoc.TableRow, column matter.TableColumn) (*types.DataType, error) {
	...
	cell := row.Cell(i)
	cellElements := ti.Doc.Iterator().Iterate(cell, cell.Children()).List()
	...
	for _, el := range cellElements {
		...
		case *asciidoc.CrossReference:
			if len(v.Elements) > 0 {
				...
				if len(name) == 0 {
					name = d.anchorId(d.Iterator(), v, v, v.ID)
				}
				sb.WriteString(name)
			}
			source = el
