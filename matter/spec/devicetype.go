...
func toDeviceTypes(spec *Specification, d *Doc, s *asciidoc.Section, pc *parseContext) (err error) {
	...
	for s := range parse.Skim[*asciidoc.Section](d.Iterator(), s, s.Children()) {
	...
	for _, dt := range deviceTypes {
		...
		elements := parse.FindAll[*asciidoc.Section](d.Iterator(), s)
...
func (d *Doc) toBaseDeviceType() (baseDeviceType *matter.DeviceType, err error) {
	for top := range parse.Skim[*asciidoc.Section](d.Iterator(), d, d.Children()) {
		err = AssignSectionTypes(d, top)
		if err != nil {
			return
		}
		var baseClusterRequirements, elementRequirements *asciidoc.Section
		parse.Search(d.Iterator(), top, top.Children(), func(sec *asciidoc.Section, parent asciidoc.Parent, index int) parse.SearchShould {
