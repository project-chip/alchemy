...
func (a *Anchor) Identifier() string {
	return a.Document.anchorId(a.Document.Iterator(), a.Parent, a.Element, a.ID)
}
...
func (doc *Doc) findAnchors() {
	...
	parse.Search(doc.Iterator(), doc, doc.Children(), func(el any, parent asciidoc.Parent, index int) parse.SearchShould {
		var anchor *Anchor
		var label string
		switch el := el.(type) {
		case *asciidoc.Anchor:
		...
		if anchor != nil {
			anchorID := doc.anchorId(doc.Iterator(), anchor.Parent, anchor.Element, anchor.ID)
...
func (d *Doc) FindAnchorByID(id asciidoc.Elements, element asciidoc.ParentElement, source log.Source) *Anchor {
	anchorID := d.anchorId(d.Iterator(), element, element, id)
