...
func NewIncludeFilter(spec *spec.Specification, paths []string) *Filter {
	return &Filter{spec: spec, paths: paths}
}

func NewExcludeFilter(spec *spec.Specification, paths []string) *Filter {
	return &Filter{spec: spec, paths: paths, exclude: true}
}

func (p Filter) Name() string {
	return ""
}

func (p *Filter) Process(cxt context.Context, inputs []*pipeline.Data[*asciidoc.Document]) (outputs []*pipeline.Data[*asciidoc.Document], err error) {
	if len(p.paths) == 0 {
		return inputs, nil
	}
